...
export default function RootLayout() {
          ...
          if (firebaseUser) {
            ...
            if (!userSynced && !syncingRef.current) {
              ...
              syncUser()
                .then(() => {
                  console.log(
                    "User synced successfully, setting userSynced to true"
                  );
  ...
  useEffect(() => {
    ...
    if (!checkingAuth && !initializing && firebaseUser) {
      console.log(
        "Navigation effect triggered. UserSynced:",
        userSynced,
        "Pathname:",
        pathname
      );
      ...
      if (userSynced) {
        ...
        console.log(
          "Navigation effect - Current route segments:",
          segments,
          "pathname:",
          pathname
        );
        ...
        if (pathname === "/login" || pathname === "/signup") {
          console.log(
            `Redirecting from ${pathname} to /(app) after successful login`
          );
          router.replace("/(app)");
        } else if (currentRoute && currentRoute !== "(app)") {
          console.log(
            `Redirecting authenticated user from ${currentRoute} to /(app)`
          );
          router.replace("/(app)");
        ...
        console.log(
          "Authenticated user not synced, waiting for sync to complete"
        );
    ...
    if (!checkingAuth && !initializing && !firebaseUser && !logoutRef.current) {
      const currentRoute = segments[0];
      console.log(
        "Logout navigation - current route segments:",
        segments,
        "pathname:",
        pathname
      );
    ...
    const timeoutId = setTimeout(() => {
      ...
      if (!checkingAuth && !syncingRef.current) {
        ...
        if (firebaseUser && userSynced) {
          ...
          console.log(
            "Authenticated user with synced data - no redirect needed from route protection"
          );
          ...
          if (
            ...
            console.log(
              `Unauthenticated user on protected route (${pathname}, segments: ${segments}), redirecting to /home`
            );
            setTimeout(() => {
              router.replace("/home");
            }, 0);
          ...
          // Unauthenticated user on public routes -> allow (do nothing)
            ...
            console.log(
              `Unauthenticated user on valid public route: ${pathname}`
            );
  ...
  useEffect(() => {
    ...
    if (checkingAuth && initializing) {
      ...
      if (
        ...
        firebaseUser &&
        (currentRoute === "(app)" || pathname.startsWith("/(app)"))
      ) {
